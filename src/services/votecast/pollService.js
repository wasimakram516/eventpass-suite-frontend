...
export const getPolls = withApiHandler(async (businessSlug = "", status = "") => {
...
// Create a new poll
export const createPoll = withApiHandler(async (pollData) => {
  const { data } = await api.post("/votecast/polls", pollData);
  return data.data;
});

// Clone a poll
export const clonePoll = withApiHandler(async (pollId) => {
  const { data } = await api.post(`/votecast/polls/${pollId}/clone`);
  return data.data;
});

// Update an existing poll
export const updatePoll = withApiHandler(async (id, updatedData) => {
  const { data } = await api.put(`/votecast/polls/${id}`, updatedData);
  return data.data;
});

// Delete a poll
export const deletePoll = withApiHandler(async (id) => {
  const { data } = await api.delete(`/votecast/polls/${id}`);
  return data.data;
});

// Vote on a poll (public)
export const voteOnPoll = withApiHandler(async (pollId, optionIndex) => {
  const { data } = await api.post(`/votecast/polls/${pollId}/vote`, { optionIndex });
  return data.data;
});

// Reset votes for a business and optional status
export const resetVotes = withApiHandler(async (businessSlug, status = "") => {
  const { data } = await api.post(`/votecast/polls/reset`, { businessSlug, status });
  return data;
});
...
export const exportPollsToExcel = async (businessSlug, status = "") => {
  try {
    ...
    link.href = url;
    link.setAttribute("download", `${businessSlug}-polls-${status || "all"}.xlsx`);
