...
export function EventsPage({
  ...
  const canView = useHasPermission(moduleKey, "view");
  ...
  const handleConfirmClone = async (selected) => {
    ...
    const clonePayments = false;

    const res = await cloneEvent(eventToClone._id, {
      cloneRegistrations,
      cloneWalkIns,
      clonePayments,
    });
  ...
  const handleSubmitEvent = async (formData, isEdit) => {
    let res;
    if (isEdit) {
      res = await updateEvent(selectedEvent._id, formData);
      if (!res?.error) {
            ...
            {filteredEvents.map((ev) => {
              ...
              return (
                <EventCardBase
                  ...
                  t={t}
                  status={status}
                  showRegistrations={showEventLinks}
                  showAudit={true}
                  locale={language === "ar" ? "ar-SA" : "en-GB"}
                  onView={showEventLinks && canView ? () =>
                    router.replace(
                      `${routeBase}/${ev.slug}${viewRouteSuffix}`
                    )
                  : undefined}
                  onClone={canCreate ? () => {
                    setEventToClone(ev);
        ...
        <ShareLinkModal
          ...
          url={
            typeof window !== "undefined" && eventToShare?.slug
              ? `${window.location.origin}${getPublicEventUrl(eventToShare)}`
              : ""
          }
