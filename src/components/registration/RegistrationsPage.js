...
import RegistrationPaymentFilters from "@/components/checkout/RegistrationPaymentFilters";
...
export function RegistrationsPage({
  ...
  registrationService,
  getEventBySlug,
  getPaymentLinkForRegistration = null,
  routeBase,
  ...
  const canEdit = useHasPermission(moduleKey, "edit");
  const canDelete = useHasPermission(moduleKey, "delete");
  const canPrint = useHasPermission(moduleKey, "print");
  const canExport = useHasPermission(moduleKey, "export");
  const canBulkImport = useHasPermission(moduleKey, "bulk_import");
  const canRecordExternalPayment = useHasPermission(moduleKey, "record_external_payment");
  const canSendEmail = useHasPermission(moduleKey, "send_email");
  const canSendWhatsapp = useHasPermission(moduleKey, "send_whatsapp");
  const canApprove = useHasPermission(moduleKey, "approve");
  const canReject = useHasPermission(moduleKey, "reject");
  const canViewPromoCodes = useHasPermission(moduleKey, "view_promo_codes");
  const canViewInvoices = useHasPermission(moduleKey, "export_payments");
  ...
  const fetchData = async () => {
    ...
    const evRes = await getEventBySlug(eventSlug);
  ...
  return (
    <Container dir={dir} maxWidth={false} disableGutters>
      ...
      >
        ...
        {paymentEnabled && canViewPromoCodes && (
          <Button
            variant="outlined"
            startIcon={<ICONS.promoCode />}
            onClick={() => router.push(`${routeBase}/${eventSlug}/promo-codes`)}
            sx={getStartIconSpacing(dir)}
          >
            {t.promoCodes}
          </Button>
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* Keep the original EventReg payment-card markup for Checkout too. */}
                    {paymentEnabled && (() => {
                      ...
                      return (
                        <Box sx={{ mt: 0.3 }}>
                          <Typography variant="caption" sx={{ display: "flex", alignItems: "center", gap: 0.6, fontWeight: 500 }}>
                            {entry.icon}
                            {entry.statusIcon}
                            <Box component="span" sx={{ color: entry.color }}>{entry.label}</Box>
                            {ps === "pending" && getPaymentLinkForRegistration && (
                              <Tooltip title={copiedPaymentLinkId === reg._id ? t.paymentLinkCopied : t.copyPaymentLink}>
                                <IconButton
                                  size="small"
                                  onClick={async () => {
                                    const result = await getPaymentLinkForRegistration(reg._id);
                                    if (!result?.error && result?.paymentUrl) {
                                      await navigator.clipboard.writeText(result.paymentUrl);
                                      setCopiedPaymentLinkId(reg._id);
                                      setTimeout(() => setCopiedPaymentLinkId((previous) => previous === reg._id ? null : previous), 2000);
                                    }
                                  }}
                    ...
                    {paymentEnabled && canViewInvoices && reg.paymentStatus === "paid" && (
                      <Button
                        size="small"
                        variant="text"
                        disabled={loadingInvoiceId === reg._id}
                        startIcon={loadingInvoiceId === reg._id ? <CircularProgress size={14} /> : <ICONS.receipt sx={{ fontSize: 16 }} />}
      ...
      <RegistrationModal
        ...
        canRecordExternalPayment={paymentEnabled && canRecordExternalPayment}
        externalPaymentUploadModule={moduleKey}
        {...registrationModalProps}
        mode="edit"
        event={eventDetails}
      />
          ...
          {paymentEnabled && (
            <RegistrationPaymentFilters
              ...
              labels={{
                ...
                allPaymentStatuses: t.all,
                allTicketTypes: t.all,
                paid: t.paymentPaid,
                pending: t.paymentPending,
                external: t.paymentExternal,
                cancelled: t.paymentCancelled,
