...
export default function EventDetails({
  ...
  useEffect(() => {
    if (bookingRedirect) window.location.href = bookingRedirect;
  }, [bookingRedirect]);
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      if (!result?.error) {
        ...
        // the session.
        const visitedKey = `event_lang_visited_${registrationRouteBase}_${eventSlug}`;
        const defaultLang = result.defaultLanguage === "ar" ? "ar" : "en";
        if (!sessionStorage.getItem(visitedKey)) {
          sessionStorage.setItem(visitedKey, "1");
          if (lang !== defaultLang) {
            router.replace(`${registrationRouteBase}/${defaultLang}/event/${eventSlug}`);
            return;
          }
        }
        ...
        // Paid events migrated from EventReg are intentionally no longer
        // returned by its endpoint. The API tells us when the same slug is
        // Checkout-owned so long-lived EventReg posters, emails and QR codes
        // continue to reach the event instead of showing a false 404.
        if (checkoutRedirectBase && result.redirectToCheckout) {
          const checkoutSlug = result.slug || eventSlug;
          router.replace(
            `${checkoutRedirectBase}/${lang}/event/${checkoutSlug}${queryString ? `?${queryString}` : ""}`,
          );
          return;
        }
      ...
      >
        ...
        {showEventDetails ||
          ...
          <EventWelcomeCard
            ...
            actionRoute={`${registrationRouteBase}/${lang}/event/${eventSlug}/register`}
            isArabic={isArabic}
          />
        ) : (
          <Button
