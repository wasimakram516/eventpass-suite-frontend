...
export default function Registration({
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      ...
      if (!result?.error) {
        setEvent(result);
      } else if (checkoutRedirectBase && result.redirectToCheckout) {
        // Keep legacy EventReg registration links valid after an event is
        // reclassified into Checkout. Preserve any query parameters supplied
        // by a campaign or QR code as well as the selected language.
        const checkoutSlug = result.slug || eventSlug;
        router.replace(
          `${checkoutRedirectBase}/${lang}/event/${checkoutSlug}/register${queryString ? `?${queryString}` : ""}`,
        );
        return;
      }
      setLoading(false);
    ...
    fetchEvent();
  }, [eventSlug, lang, getEventBySlug, checkoutRedirectBase, queryString, router]);
  ...
  const handleSubmit = async () => {
    ...
    if (checkoutEnabled && event?.isPaid && !selectedTicketTypeId) {
    ...
    if (checkoutEnabled && event?.isPaid) {
  ...
  const handleDialogClose = () => {
    setShowDialog(false);
    router.replace(`${eventRouteBase}/${lang}/event/${eventSlug}`);
  };
  ...
  const handleConfirmPayment = async () => {
    ...
    if (!result?.error && data?.skipPayment) {
      ...
      window.location.href = `${paymentRouteBase}/event/${eventSlug}/payment/success?registration_id=${registrationId}&lang=${lang}`;
      return;
    }
    ...
    >
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 4, width: '100%', maxWidth: 1040, justifyContent: 'center', alignItems: 'flex-start' }}>
        ...
        >
          ...
          {checkoutEnabled && event?.isPaid && event?.ticketTypes?.length > 0 && (
            <Box sx={{ mb: 3 }}>
              <TicketTypeSelector
                ...
                onChange={(newTicketId) => {
                  ...
                  setFormData((previous) => {
                    const cleared = {};
                    (event?.globalDependentFields || []).forEach(({ inputName }) => {
                      if (inputName in previous) cleared[inputName] = "";
                    });
                    return { ...previous, ...cleared };
                  });
                }}
                label={`${t.selectTicket} *`}
                searchPlaceholder={isArabic ? "بحث عن تذكرة…" : "Search tickets…"}
                soldOutLabel={t.ticketSoldOut}
          ...
          >
            {submitting ? <CircularProgress size={22} /> : checkoutEnabled && event?.isPaid ? t.proceedToPayment : t.submit}
          </Button>
          ...
          {paymentBreakdown && (
            <Box>
              ...
              <PaymentBreakdown
                ...
                ticketName={selectedTicket?.name || t.ticketLabel}
                currency={t.omr}
                labels={{
                  ticket: t.ticketLabel,
                  ...
                  securePayment: paymentBreakdown.total <= 0 ? t.freeNote : t.securePayment,
                }}
              />
