...
const drawConfidentialFooter = (page, pageWidth, font, language) => {
  const fontSize = 8;
  ...
  if (language === "ar") {
    ...
    const arabicWidth = font.widthOfTextAtSize(arabicPart, fontSize);
    const englishWidth = font.widthOfTextAtSize(englishPart, fontSize);
    const totalWidth = arabicWidth + englishWidth;
...
export const exportChartsToPDF = async (
  ...
  // Header and Branding
  try {
    const wwLogoUrl = "/WWDS.png";
    const logoResponse = await fetch(wwLogoUrl);
  ...
  } catch (e) { console.warn("Could not load branding logo"); }
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    drawConfidentialFooter(p, pageWidth, font, activeLanguage);
  }
