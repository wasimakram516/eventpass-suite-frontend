...
import VoteCastEventModal from "@/components/modals/VoteCastEventModal";
...
import {
    getAllVoteCastEventsByBusiness,
    createVoteCastEvent,
    updateVoteCastEvent,
    deleteVoteCastEvent,
} from "@/services/votecast/eventService";
...
const translations = {
    en: {
        pageTitle: "Manage VoteCast Events",
        pageDescription: "Manage all VoteCast events for this business.",
    ...
    ar: {
        ...
        pageDescription: "إدارة جميع فعاليات VoteCast لهذا العمل.",
...
export default function EventsPage() {
    ...
    useEffect(() => {
        ...
        const fetchEvents = async () => {
            setLoading(true);
            const result = await getAllVoteCastEventsByBusiness(selectedBusiness);
    ...
    const handleSubmitEvent = async (formData, isEdit) => {
        let res;
        if (isEdit) {
            res = await updateVoteCastEvent(selectedEvent._id, formData);
            if (!res?.error) {
                setEvents((prev) => prev.map((e) => (e._id === res._id ? res : e)));
                handleCloseModal();
            }
        } else {
            res = await createVoteCastEvent(formData);
            if (!res?.error) {
    ...
    const handleDeleteEvent = async () => {
        const res = await deleteVoteCastEvent(eventToDelete._id);
        if (!res?.error) {
    ...
    const filteredEvents = useMemo(() => {
        ...
        return events.filter((ev) => {
            ...
            const slugVal = (ev.slug || "").toLowerCase();
                            ...
                            return (
                                <Grid item xs={12} sm={6} md={4} key={ev._id}>
                                    <EventCardBase
                                        ...
                                        onViewResults={() =>
                                            router.push(
                                                `/cms/modules/votecast/events/${ev.slug}/results`
                                            )
                                        }
                                    />
                                </Grid>
                ...
                <ShareLinkModal
                    ...
                    url={
                        typeof window !== "undefined" && eventToShare?.slug
                            ? `${window.location.origin}/votecast/${eventToShare.slug}`
                            : ""
                    }
